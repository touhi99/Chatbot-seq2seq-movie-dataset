...
for question in clean_questions:
	for word in question.split():
		if word not in word2count:
			...
			word2count[word] += 1
# Can be improved rather than both q and a into dictionary
...
def preprocess_targets(targets, word2int, batch_size):
	...
	prepcessed_targets = tf.concat([left_side, right_side], 1)
	return prepcessed_targets 
...
def decode_training_set(encoder_state, decoder_cell, 
	decoder_embedded_inputs, sequence_length, 
	...
	decoder_output, decoder_final_state, decoder_final_context_state = tf.contrib.seq2seq.dynamic_rnn_decoder(
		decoder_cell, training_decoder_function,
		decoder_embedded_inputs, sequence_length,
...
#Splitting the data to batch of q and a
	...
	for batch_index, (padded_questions_in_batch, padded_answers_in_batch) in enumerate(split_into_batches(training_questions, training_answers, batch_size)):
		...
		if batch_index % batch_index_check_validation_loss == 0 and batch_index > 0:
			...
			if average_validation_loss_error <= min(list_validation_loss_error):
				...
				print("Sorry I am dumb yet")
				early_stopping_check += 1
				if early_stopping_check == early_stopping_stop:
					break
	if early_stopping_check == early_stopping_stop:
		print("Sorry, I am dumbest")
		break
print("GameOver")
